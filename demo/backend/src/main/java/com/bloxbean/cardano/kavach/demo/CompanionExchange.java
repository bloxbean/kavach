...
final class CompanionExchange {
    ...
    CompanionExchange(Path identity) { this.identity = identity; }
    ...
    Map<String, Object> pairing() throws Exception {
        byte[] secret = key();
        try {
            byte[] pub = new byte[32]; Ed25519.generatePublicKey(secret, 0, pub, 0);
            var pair = Map.of("version", 1, "kind", "pair", "name", "Kavach dashboard", "publicKey", hex(pub));
            byte[] fingerprint = new byte[32]; var hash = new Blake2bDigest(256);
            hash.update(pub, 0, pub.length); hash.doFinal(fingerprint, 0);
    ...
    Map<String, Object> request(UUID ticket, String profile, String cbor, String stateCbor,
        ...
        try {
            ...
            byte[] signature = new byte[64]; Ed25519.sign(secret, 0, message, 0, message.length, signature, 0);
            ...
            try {
                deflater.setInput(encoded); deflater.finish(); byte[] compressed = new byte[9216];
    ...
    static byte[] verify(Map<String, Object> response, UUID ticket, String profile, int id,
        ...
        if (expiresAt <= Instant.now().toEpochMilli()) throw new IllegalArgumentException("Companion request expired; export a fresh request");
