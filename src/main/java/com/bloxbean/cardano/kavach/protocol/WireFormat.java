...
public final class WireFormat {
     ...
     */
    public static byte[] protocolTag() {
        return TAG.clone();
    }
    ...
    public static String renderState(PlutusData state) {
        validateState(state);
        var decoder = new WireFormat();
        var f = record(state, 0, 12);
        decoder.line("state.schemaVersion", "1");
        decoder.account(f.get(1), "state.account");
        decoder.deployment(f.get(2), "state.deployment");
        decoder.core(f.get(3), "state.core");
        decoder.number(f.get(4), "state.version", MAX);
        decoder.module(f.get(5), "state.module");
        decoder.config(f.get(6), "state.config");
    ...
    public static void validateState(PlutusData data) {
        ...
        var decoder = new WireFormat();
        var f = record(data, 0, 12);
        exact(f.get(0), 1);
        decoder.account(f.get(1), "account");
        decoder.deployment(f.get(2), "deployment");
        decoder.core(f.get(3), "core");
        uint(f.get(4), MAX);
        decoder.module(f.get(5), "module");
        ...
        uint(f.get(10), MAX);
        var mode = constructor(f.get(11));
        if (mode.tag() == 0 || mode.tag() == 1) {
            record(mode, mode.tag(), 0);
            require(Builtins.serialiseData(data).length <= MAX_NORMAL_STATE_BYTES, "normal/frozen state size");
        } else {
            var pending = record(mode, 2, 3);
            bytes(pending.get(0), 32);
            uint(pending.get(1), MAX);
     ...
     */
    public static void validateConfig(PlutusData data) {
        new WireFormat().config(data, "config");
    }
    ...
    private static byte[] hashEncoded(byte[] encoded) {
        ...
        hash.update(encoded, 0, encoded.length);
    ...
    private void domain(PlutusData data, String path) {
        var f = record(data, 0, 6);
        exact(f.get(0), 1);
        line(path + ".protocolVersion", "1");
        deployment(f.get(1), path + ".deployment");
        account(f.get(2), path + ".account");
        core(f.get(3), path + ".core");
        number(f.get(4), path + ".stateVersion", MAX);
        input(f.get(5), path + ".stateRef");
    }

    private void deployment(PlutusData data, String path) {
        var f = record(data, 0, 3);
        number(f.get(0), path + ".networkId", BigInteger.ONE);
        number(f.get(1), path + ".networkMagic", new BigInteger("4294967295"));
        hex(f.get(2), path + ".deploymentId", 32);
    }

    private void account(PlutusData data, String path) {
        var f = record(data, 0, 2);
        hex(f.get(0), path + ".policy", 28);
        hex(f.get(1), path + ".name", 0);
    }

    private void core(PlutusData data, String path) {
        var f = record(data, 0, 3);
        String[] names = {"stateValidator", "assetValidator", "checkpoint"};
    ...
    private void module(PlutusData data, String path) {
        var f = record(data, 0, 2);
        hex(f.get(0), path + ".scriptHash", 28);
        exact(f.get(1), 1);
        line(path + ".abiVersion", "1");
    }

    private void validity(PlutusData data) {
        var f = record(data, 0, 2);
        var lower = number(f.get(0), "validity.notBeforeInclusive", MAX);
    ...
    private void action(PlutusData data) {
        ...
        require(tag >= 0 && tag < names.length, "action tag");
        line("action", names[tag]);
        int[] counts = {3, 1, 2, 0, 0, 2, 2, 2, 4};
        var f = record(c, tag, counts[tag]);
        switch (tag) {
            case 0 -> spend(f);
            case 1 -> config(f.get(0), "action.newConfig");
            case 2 -> {
                module(f.get(0), "action.newModule");
                config(f.get(1), "action.newConfig");
            }
            case 3, 4 -> {
            }
            case 5 -> {
                number(f.get(0), "action.nextSequence", MAX);
                require(uint(f.get(0), MAX).signum() > 0, "next sequence");
                config(f.get(1), "action.replacementConfig");
            }
            case 6 -> {
                require(number(f.get(0), "action.currentSequence", MAX).signum() > 0, "current sequence");
                hex(f.get(1), "action.proposalCommitment", 32);
            }
            case 7 -> {
                require(number(f.get(0), "action.currentSequence", MAX).signum() > 0, "current sequence");
                config(f.get(1), "action.replacementConfig");
            ...
            case 8 -> {
                input(f.get(0), "action.input");
                number(f.get(1), "action.recipientIndex", BigInteger.valueOf(15));
                ...
                byte[] expected = bytes(f.get(3), 32);
                hex(f.get(3), "action.inputValueDigest", 32);
    ...
    private void spend(List<PlutusData> f) {
        var inputs = list(f.get(0), 1, 8);
        byte[] previousHash = null;
        BigInteger previousIndex = null;
        for (int i = 0; i < inputs.size(); i++) {
            var ref = record(inputs.get(i), 0, 2);
            byte[] hash = bytes(ref.get(0), 32);
            BigInteger index = uint(ref.get(1), BigInteger.valueOf(65535));
            ...
            input(inputs.get(i), "action.inputs[" + i + "]");
            previousHash = hash;
            previousIndex = index;
        }
        var recipients = list(f.get(1), 0, 8);
        int previous = -1;
        Set<String> union = new HashSet<>();
        for (int i = 0; i < recipients.size(); i++) {
            var r = record(recipients.get(i), 0, 3);
            String path = "action.recipients[" + i + "]";
            int index = number(r.get(0), path + ".index", BigInteger.valueOf(15)).intValueExact();
            require(index > previous, "recipient order/duplicate");
            previous = index;
        ...
        require(union.size() <= 12, "recipient asset union");
        number(f.get(2), "action.maxAccountFeeLovelace", BigInteger.valueOf(5_000_000));
    }

    private void input(PlutusData data, String path) {
        var f = record(data, 0, 2);
        hex(f.get(0), path + ".txId", 32);
        number(f.get(1), path + ".index", BigInteger.valueOf(65535));
    }

    private void address(PlutusData data, String path) {
        var f = record(data, 0, 2);
        credential(f.get(0), path + ".payment");
        var stake = constructor(f.get(1));
        if (stake.tag() == 1) {
            record(stake, 1, 0);
            line(path + ".stake", "none");
        } else {
            var present = record(stake, 0, 1);
            var cred = constructor(present.get(0));
            if (cred.tag() == 0) credential(record(cred, 0, 1).get(0), path + ".stake.hash");
            else {
                var ptr = record(cred, 1, 3);
                for (int i = 0; i < 3; i++) number(ptr.get(i), path + ".stake.pointer[" + i + "]", MAX);
            }
        }
    }
    ...
    private void credential(PlutusData data, String path) {
        var c = constructor(data);
        require(c.tag() == 0 || c.tag() == 1, "credential tag");
        line(path + ".kind", c.tag() == 0 ? "key" : "script");
        hex(record(c, c.tag(), 1).get(0), path + ".hash", 28);
    }

    private void value(PlutusData data, String path, Set<String> union) {
        var entries = list(data, 1, 12);
        byte[] previousPolicy = null;
        byte[] previousName = null;
        for (int i = 0; i < entries.size(); i++) {
            var f = record(entries.get(i), 0, 3);
            byte[] policy = anyBytes(f.get(0));
            byte[] name = anyBytes(f.get(1));
            ...
            line(path + "[" + i + "].policy", HexFormat.of().formatHex(policy));
            line(path + "[" + i + "].name", HexFormat.of().formatHex(name));
            require(number(f.get(2), path + "[" + i + "].quantity", MAX).signum() > 0, "zero output asset");
            union.add(HexFormat.of().formatHex(policy) + ":" + HexFormat.of().formatHex(name));
    ...
    private void resolvedValue(PlutusData value) {
        ...
        for (var policyEntry : policies) {
            ...
            require(!tokens.isEmpty() && tokens.size() <= 2048, "ledger token count");
            byte[] previousName = null;
            for (var token : tokens) {
                require(++count <= 2048, "ledger value entry count");
                byte[] name = anyBytes(token.key());
                ...
                line(path + ".policy", HexFormat.of().formatHex(policy));
                line(path + ".name", HexFormat.of().formatHex(name));
                require(number(token.value(), path + ".quantity", BigInteger.ONE.shiftLeft(64).subtract(BigInteger.ONE)).signum() > 0, "ledger value quantity");
                previousName = name;
            }
            previousPolicy = policy;
        }
    ...
    private void config(PlutusData data, String path) {
        ...
        if (PolicyConfigCodec.isPolicy(data)) {
            ...
            for (int i = 0; i < ids.size(); i++)
                number(ids.get(i), path + ".coseIds[" + i + "]", BigInteger.valueOf(15));
            ...
            for (int i = 0; i < members.size(); i++)
                number(members.get(i), path + ".smallSpend.keys[" + i + "]", BigInteger.valueOf(15));
            return;
        }
        require(Builtins.serialiseData(data).length <= MAX_CONFIG_BYTES, "config size");
        var f = record(data, 0, 8);
        exact(f.get(0), 1);
        line(path + ".schemaVersion", "1");
        var keys = list(f.get(1), 3, 16);
        Set<Integer> registry = new HashSet<>();
        Set<String> publicKeys = new HashSet<>();
        int previous = -1;
        for (int i = 0; i < keys.size(); i++) {
            var key = record(keys.get(i), 0, 2);
            int id = number(key.get(0), path + ".keys[" + i + "].id", BigInteger.valueOf(15)).intValueExact();
            require(id > previous && registry.add(id), "key order/duplicate");
            previous = id;
            String hex = hex(key.get(1), path + ".keys[" + i + "].publicKey", 32);
            require(publicKeys.add(hex), "public key alias");
            require(Ed25519.validatePublicKeyFull(bytes(key.get(1), 32), 0), "invalid Ed25519 public key");
        }
        String[] roles = {"spend", "admin", "freeze", "unfreeze", "recovery", "cancel"};
        List<Set<Integer>> sets = new ArrayList<>();
        for (int i = 0; i < roles.length; i++) {
            var policy = record(f.get(i + 2), 0, 2);
            var members = list(policy.get(1), 1, 8);
            int threshold = number(policy.get(0), path + "." + roles[i] + ".threshold", BigInteger.valueOf(8)).intValueExact();
            require(threshold >= 1 && threshold <= members.size(), "threshold");
            Set<Integer> ids = new HashSet<>();
            previous = -1;
            for (int j = 0; j < members.size(); j++) {
                int id = number(members.get(j), path + "." + roles[i] + ".keys[" + j + "]", BigInteger.valueOf(15)).intValueExact();
                require(id > previous && registry.contains(id), "policy key order/unknown key");
                previous = id;
                ids.add(id);
            }
            sets.add(ids);
        ...
        var admin = record(f.get(3), 0, 2);
        var threshold = uint(admin.get(0), BigInteger.valueOf(8)).intValueExact();
        int everydayAdminKeys = 0;
        for (int id : sets.get(0)) if (sets.get(1).contains(id)) everydayAdminKeys++;
    ...
    private void line(String path, String value) {
        display.append(path).append('=').append(value).append('\n');
    }

    private String hex(PlutusData d, String path, int length) {
        String hex = HexFormat.of().formatHex(bytes(d, length));
        line(path, hex);
        return hex;
    }

    private BigInteger number(PlutusData d, String path, BigInteger max) {
        var n = uint(d, max);
        line(path, n.toString());
        return n;
    }

    private static void exact(PlutusData d, long n) {
        require(uint(d, MAX).equals(BigInteger.valueOf(n)), "version");
    }

    private static void range(PlutusData d, BigInteger min, BigInteger max) {
        require(uint(d, max).compareTo(min) >= 0, "minimum");
    }

    private static BigInteger uint(PlutusData d, BigInteger max) {
        require(d instanceof PlutusData.IntData, "integer type");
        var n = ((PlutusData.IntData) d).value();
        require(n.signum() >= 0 && n.compareTo(max) <= 0, "integer bound");
        return n;
    }

    private static byte[] bytes(PlutusData d, int length) {
        var bytes = anyBytes(d);
        require(bytes.length == length, "byte length");
        return bytes;
    }

    private static byte[] anyBytes(PlutusData d) {
        require(d instanceof PlutusData.BytesData, "bytes type");
        return ((PlutusData.BytesData) d).value();
    }

    private static PlutusData.ConstrData constructor(PlutusData d) {
        require(d instanceof PlutusData.ConstrData, "constructor type");
        var c = (PlutusData.ConstrData) d;
        require(c.constructorTag().signum() >= 0 && c.constructorTag().compareTo(BigInteger.valueOf(8)) <= 0, "constructor range");
        return c;
    }

    private static List<PlutusData> record(PlutusData d, int tag, int count) {
        var c = constructor(d);
        require(c.constructorTag().equals(BigInteger.valueOf(tag)) && c.fields().size() == count, "constructor tag/arity");
        return c.fields();
    }

    private static List<PlutusData> list(PlutusData d, int min, int max) {
        require(d instanceof PlutusData.ListData, "list type");
        var list = ((PlutusData.ListData) d).items();
        require(list.size() >= min && list.size() <= max, "list bound");
        return list;
    }
    ...
    private static void guard(PlutusData data) {
        ...
        pending.push(new Frame(data, 0));
        int nodes = 0;
        int bytes = 0;
        while (!pending.isEmpty()) {
            var frame = pending.pop();
            require(++nodes <= 4096 && frame.depth <= 32, "Data complexity");
            ...
            else if (frame.data instanceof PlutusData.BytesData b) {
                bytes += b.value().length;
                require(bytes <= 8192, "Data byte budget");
            } else if (frame.data instanceof PlutusData.IntData n)
                require(n.value().bitLength() <= 64, "Data integer size");
