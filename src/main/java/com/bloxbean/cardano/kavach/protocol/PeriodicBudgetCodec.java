...
public final class PeriodicBudgetCodec {
    ...
    public static Configuration decode(PlutusData data) {
        ...
        if (optional instanceof PlutusData.ConstrData c && c.tag() == 1 && c.fields().isEmpty())
            budget = Optional.empty();
        else {
            var b = fields(fields(optional, 0, 1).getFirst(), 0, 3);
            var id = fields(b.get(0), 0, 2);
            byte[] policy = bytes(id.get(0));
            byte[] name = bytes(id.get(1));
            require(policy.length == 28 && name.length == 0, "Budget counter identifier");
            var period = integer(b.get(1));
            var limit = integer(b.get(2));
            duration(period);
    ...
    public static Usage usage(PlutusData data) {
        ...
        var version = integer(f.get(0));
        var period = integer(f.get(1));
        var start = integer(f.get(2));
        var spent = integer(f.get(3));
        ...
        if (period.equals(BigInteger.ZERO))
            require(start.signum() == 0 && spent.signum() == 0, "Unused counter must be zero");
    ...
    private static boolean unsigned(BigInteger value) {
        return value.signum() >= 0 && value.compareTo(BigInteger.valueOf(Long.MAX_VALUE)) <= 0;
    }
    ...
    private static BigInteger integer(PlutusData data) {
        require(data instanceof PlutusData.IntData, "Budget integer");
        return ((PlutusData.IntData) data).value();
    }

    private static byte[] bytes(PlutusData data) {
        require(data instanceof PlutusData.BytesData, "Budget bytes");
