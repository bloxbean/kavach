...
public final class RecoveryModel {
    ...
    public record State(BigInteger version, BigInteger sequence, BigInteger notBefore,
        ...
        public State {
            unsigned(version); unsigned(sequence); unsigned(notBefore);
    ...
    public static State apply(State old, Action action, Authority authority, Interval interval, String commitment) {
        ...
        if (action == Action.SPEND) { require(old.mode == Mode.NORMAL); return old; }
    ...
    private static BigInteger add(BigInteger x, BigInteger y) { var result = x.add(y); unsigned(result); return result; }
    private static void unsigned(BigInteger n) { require(n != null && n.signum() >= 0 && n.compareTo(MAX) <= 0); }
    private static void require(boolean condition) { if (!condition) throw new IllegalArgumentException("Invalid recovery transition or bound"); }
