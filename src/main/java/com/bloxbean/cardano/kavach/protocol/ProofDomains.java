...
public final class ProofDomains {
    private static PlutusData tag(String value) {
        return PlutusData.bytes(value.getBytes(StandardCharsets.UTF_8));
    }

    private static PlutusData hash(PlutusData data) {
        return PlutusData.bytes(WireFormat.digest(data));
    }
    ...
    public static PlutusData configuration(PlutusData state, PlutusData intent) {
        ...
        WireFormat.renderIntent(intent);
        bind(state, intent);
    ...
    public static PlutusData recoveryCommitment(PlutusData state, PlutusData initiation) {
        WireFormat.validateState(state);
        WireFormat.renderIntent(initiation);
        bind(state, initiation);
    ...
    public static PlutusData target(PlutusData pendingState, PlutusData completion) {
        WireFormat.validateState(pendingState);
        WireFormat.renderIntent(completion);
        bind(pendingState, completion);
