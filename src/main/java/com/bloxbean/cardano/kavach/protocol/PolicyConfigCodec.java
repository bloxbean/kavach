...
public final class PolicyConfigCodec {
    ...
    public static ThresholdPolicy spendPolicy(PlutusData value, Action action) {
        ...
        for (var recipient : spend.recipients())
            for (var asset : recipient.value()) {
                if (asset.policy().length != 0 || asset.name().length != 0 || asset.quantity().signum() <= 0)
                    return base.spend();
    ...
    public static List<BigInteger> ids(ThresholdPolicy policy) {
        var ids = new ArrayList<BigInteger>();
        for (var id : policy.credentialIds()) ids.add(id);
        return List.copyOf(ids);
    }

    private static boolean containsKey(Ed25519Config config, BigInteger id) {
        for (var key : config.keys()) if (key.credentialId().equals(id)) return true;
        return false;
    }
    ...
    private static List<PlutusData> items(PlutusData value) {
        return ((PlutusData.ListData) value).items();
    }

    private static BigInteger integer(PlutusData value) {
