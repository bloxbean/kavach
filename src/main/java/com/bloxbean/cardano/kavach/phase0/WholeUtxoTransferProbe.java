...
public class WholeUtxoTransferProbe {
     ...
     */
    @Entrypoint public static boolean validate(Transfer transfer, ScriptContext ctx) {
        ...
        boolean keyRecipient = switch (transfer.recipient().credential()) { case Credential.PubKeyCredential key -> true; default -> false; };
        if (!rewarding || !keyRecipient || tx.inputs().size() > 16 || tx.outputs().size() > 16 || tx.referenceInputs().size() > 4
                ...
                || !Builtins.equalsData((PlutusData) (Object) transfer, (PlutusData) (Object) new Transfer(transfer.input(), transfer.recipientIndex(), transfer.recipient(), transfer.valueDigest()))) return false;
        ...
        if (!plain || recipient.referenceScript().isPresent() || !recipient.address().equals(transfer.recipient())) return false;
        ...
        for (var output : tx.outputs()) {
            ...
            if (BigInteger.valueOf(outputPosition) != index) {
                ...
                if (!keyOutput || !noDatum || output.referenceScript().isPresent() || !adaOnly(output.value())) valid = false;
            }
    ...
    static boolean preservesValue(Value input, Value output) {
        var incoming = Builtins.unMapData(input); var outgoing = Builtins.unMapData(output);
        ...
        BigInteger inputAda = ValuesLib.lovelaceOf(input); BigInteger outputAda = ValuesLib.lovelaceOf(output);
