...
public class ValueAccountingProbe {
     ...
     */
    @Entrypoint
    public static boolean validate(Allocation allocation, ScriptContext ctx) {
        ...
        if (!rewarding || tx.inputs().size() > 16 || tx.outputs().size() > 16 || tx.referenceInputs().size() > 4
                ...
                || !Builtins.equalsData((PlutusData) (Object) allocation, (PlutusData) (Object) new Allocation(allocation.recipientIndices(), allocation.feeLimit())))
            return false;

        boolean valid = true;
        int accountInputs = 0;
        int inputNativeEntries = 0;
        JulcList<Delta> deltas = JulcList.empty();
        BigInteger inputAda = BigInteger.ZERO;
        BigInteger allocatedAda = BigInteger.ZERO;
        for (var input : tx.inputs()) {
            if (input.resolved().address().equals(accountAddress)) {
                ...
                for (AssetEntry asset : entries) {
                    ...
                    if (amount.signum() < 0 || amount.compareTo(BigInteger.valueOf(9223372036854775807L)) > 0)
                        valid = false;
                    if (Builtins.lengthOfByteString(asset.policyId()) == 0) inputAda = inputAda.add(amount);
                    else {
                        inputNativeEntries = inputNativeEntries + 1;
                        if (inputNativeEntries <= 12)
                            additions = additions.prepend(new Delta(asset.policyId(), asset.tokenName(), amount, BigInteger.ZERO));
                    }
                }
        ...
        for (BigInteger index : allocation.recipientIndices()) {
            if (index.compareTo(previous) <= 0 || index.signum() < 0 || index.compareTo(BigInteger.valueOf(tx.outputs().size())) >= 0)
                valid = false;
            previous = index;
        }
        int position = 0;
        ...
        for (var output : tx.outputs()) {
            boolean change = output.address().equals(accountAddress);
            boolean recipient = false;
            for (BigInteger index : allocation.recipientIndices())
                if (index == BigInteger.valueOf(position)) recipient = true;
            ...
            for (AssetEntry asset : entries) {
                ...
                if (change || recipient) {
                    ...
                    else if (nativeEntries <= 12)
                        additions = additions.prepend(new Delta(asset.policyId(), asset.tokenName(), BigInteger.ZERO, amount));
                }
            }
            if (!ordered(additions)) valid = false;
            deltas = merge(deltas, additions);
        ...
        if (!valid || accountInputs < 1 || accountInputs > 8 || nativeEntries > 12 || inputNativeEntries > 12)
            return false;
        ...
        if (inputAda.compareTo(BigInteger.valueOf(9223372036854775807L)) > 0 || allocatedAda.compareTo(BigInteger.valueOf(9223372036854775807L)) > 0
                || adaDifference.signum() < 0 || adaDifference.compareTo(feeLimit) > 0 || adaDifference.compareTo(tx.fee()) > 0)
            return false;
        for (Delta delta : deltas) {
            BigInteger credit = delta.credit();
            BigInteger debit = delta.debit();
            if (credit != debit || credit.compareTo(BigInteger.valueOf(9223372036854775807L)) > 0 || debit.compareTo(BigInteger.valueOf(9223372036854775807L)) > 0)
                valid = false;
        }
    ...
    static JulcList<Delta> merge(JulcList<Delta> a, JulcList<Delta> b) {
        ...
        BigInteger credit = first.credit();
        BigInteger debit = first.debit();
