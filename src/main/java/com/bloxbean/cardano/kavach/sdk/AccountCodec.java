...
public final class AccountCodec {
    ...
    public static PlutusData data(Object value) {
        return switch (value) {
            ...
            case IntentDomain d ->
                    record(0, d.protocolVersion(), d.deploymentDomain(), d.accountId(), d.coreBinding(), d.stateVersion(), d.stateRef());
            ...
            case TransferWholeUtxo t ->
                    record(8, t.accountInput(), t.recipientIndex(), t.recipientAddress(), t.inputValueDigest());
            ...
            case ModuleRedeemer m ->
                    record(0, m.abiVersion(), m.intent(), m.operationProof(), list(m.configPossession()), list(m.receipts()));
            case ModuleRegistration r -> record(1);
            case GenesisModuleRedeemer g ->
                    record(2, g.abiVersion(), g.state(), list(g.configPossession()), list(g.receipts()));
            case AssetRedeemer a -> record(0, a.abiVersion(), a.intentDigest());
            case GenesisProofEnvelope g ->
                    record(0, g.tag(), g.deploymentDomain(), g.accountId(), g.coreBinding(), g.authModule(), g.configDigest());
            case KeyEntry k -> record(0, k.credentialId(), k.publicKey());
            case ThresholdPolicy p -> record(0, p.threshold(), list(p.credentialIds()));
            case Ed25519Config c ->
                    record(0, c.schemaVersion(), list(c.keys()), c.spend(), c.admin(), c.freeze(), c.unfreeze(), c.recovery(), c.cancel());
            case PolicyConfig c ->
                    record(0, c.schemaVersion(), c.roles(), list(c.coseIds()), c.smallPaymentLimit(), c.smallSpend());
    ...
    public static AccountState decodeState(PlutusData encoded) {
        ...
        AccountMode decodedMode = switch (mode.tag()) {
            ...
            case 1 -> new Frozen();
            case 2 ->
                    new RecoveryPending(rawBytes(mode.fields().get(0)), rawInteger(mode.fields().get(1)), mode.fields().get(2));
    ...
    private static byte[] rawBytes(PlutusData data) {
        return ((PlutusData.BytesData) data).value();
    }

    private static BigInteger rawInteger(PlutusData data) {
