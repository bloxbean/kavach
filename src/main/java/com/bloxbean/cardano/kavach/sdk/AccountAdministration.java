    ...
    public static AccountState successor(AccountState previous, IntentEnvelope request,
        ...
        switch (request.action()) {
            case ReplaceConfig change -> {
                action = RecoveryModel.Action.CONFIGURE; role = RecoveryModel.Authority.ADMIN;
            ...
            case ReplaceModule change -> {
                ...
                module = change.newModule(); config = change.newConfig();
            }
            case Freeze ignored -> { action = RecoveryModel.Action.FREEZE; role = RecoveryModel.Authority.FREEZE; }
            case Unfreeze ignored -> { action = RecoveryModel.Action.UNFREEZE; role = RecoveryModel.Authority.UNFREEZE; }
            case StartRecovery start -> {
                action = RecoveryModel.Action.START; role = RecoveryModel.Authority.RECOVERY;
            ...
            case CancelRecovery cancel -> {
                action = RecoveryModel.Action.CANCEL; role = RecoveryModel.Authority.CANCEL;
            ...
            case CompleteRecovery complete -> {
                action = RecoveryModel.Action.COMPLETE; role = RecoveryModel.Authority.TARGET;
        ...
        AccountMode mode = switch (next.mode()) {
            ...
            case PENDING -> new RecoveryPending(HexFormat.of().parseHex(next.commitment()), next.executeAfter(), target);
        };
