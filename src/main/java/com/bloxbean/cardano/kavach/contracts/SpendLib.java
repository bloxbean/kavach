...
public class SpendLib {
    ...
    public static boolean validate(Spend spend, Address accountAddress, JulcList<RewardReceipt> receipts, ScriptContext ctx) {
        ...
        byte[] recipientMask = new byte[]{0, 0}; boolean validIndices = true;
        for (var recipient : spend.recipients()) {
            var index = recipient.outputIndex();
            if (AccountLib.lessThan(index, BigInteger.ZERO) || AccountLib.atMost(BigInteger.valueOf(16), index)) validIndices = false;
            else recipientMask = Builtins.orByteString(false, recipientMask, Builtins.shiftByteString(new byte[]{0, 1}, index.longValue()));
        }
    ...
    static boolean semantics(Spend spend, Address accountAddress, JulcList<RewardReceipt> receipts, byte[] recipientMask, ScriptContext ctx) {
        ...
        byte[] previousHash = new byte[]{}; BigInteger previousIndex = BigInteger.valueOf(-1);
        for (TxOutRef ref : spend.accountInputs()) {
            byte[] hash = ref.txId().hash();
            if (!Builtins.lessThanByteString(previousHash, hash) && !(Builtins.equalsByteString(previousHash, hash) && AccountLib.lessThan(previousIndex, ref.index()))) valid = false;
            previousHash = hash; previousIndex = ref.index();
        }
        ...
        for (var recipient : spend.recipients()) {
            ...
            if (!AccountLib.index(index, ctx) || AccountLib.atMost(index, previous) || AccountLib.receiptAt(receipts, index)
                ...
                if (!output.address().equals(recipient.address()) || !AccountLib.plain(output) || !exactValue(recipient.value(), output.value())) valid = false;
            }
            previous = index;
        }
        int position = 0;
        for (var output : ctx.txInfo().outputs()) {
            ...
            if (!account && !recipient) {
                boolean key = switch (output.address().credential()) { case Credential.PubKeyCredential credential -> true; default -> false; };
    ...
    static boolean exactValue(JulcList<Asset> signed, Value actual) {
        ...
        if (signed.isEmpty() || AccountLib.listSize((JulcList<PlutusData>)(Object)signed) > 12 || AccountLib.listSize((JulcList<PlutusData>)(Object)signed) != AccountLib.listSize((JulcList<PlutusData>)(Object)entries)) return false;
        boolean valid = true; int i = 0;
        byte[] previousPolicy = new byte[]{}; byte[] previousName = new byte[]{};
        for (var asset : signed) {
            ...
            if (i == 0) {
                if (Builtins.lengthOfByteString(asset.policy()) != 0 || Builtins.lengthOfByteString(asset.name()) != 0) valid = false;
            } else {
                if (Builtins.lengthOfByteString(asset.policy()) != 28 || Builtins.lengthOfByteString(asset.name()) > 32
                        ...
                        && !(Builtins.equalsByteString(previousPolicy, asset.policy()) && Builtins.lessThanByteString(previousName, asset.name())))) valid = false;
            }
            previousPolicy = asset.policy(); previousName = asset.name(); i = i + 1;
        }
        return valid;
    }
}
