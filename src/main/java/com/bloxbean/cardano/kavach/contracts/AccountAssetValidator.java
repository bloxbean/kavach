...
public class AccountAssetValidator {
    ...
    public static boolean validate(AssetRedeemer redeemer, ScriptContext ctx) {
        ...
        return switch (core.intent().action()) {
            case Spend spend -> !ownRef.equals(spend.accountInputs().head()) || SpendLib.validate(spend, accountAddress, core.receipts(), ctx);
            case TransferWholeUtxo whole -> ownRef.equals(whole.accountInput()) && WholeTransferLib.validate(whole, accountAddress, core.receipts(), ctx);
            default -> false;
        };
    }
}
