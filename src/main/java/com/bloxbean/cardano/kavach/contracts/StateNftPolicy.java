...
public class StateNftPolicy {
    ...
    public static boolean validate(BigInteger redeemer, ScriptContext ctx) {
        if (!redeemer.equals(BigInteger.ZERO) || !coreVersion.equals(BigInteger.ONE) || !AccountLib.transactionShape(ctx)) return false;
    ...
    static boolean initialize(ScriptContext ctx, ScriptInfo.MintingScript minting) {
        ...
        boolean signed = false; boolean consumed = false; boolean inputsValid = true;
        ...
        int matches = 0; int position = 0; int stateIndex = -1;
        for (var output : ctx.txInfo().outputs()) {
            if (!AccountLib.equalInteger(ValuesLib.assetOf(output.value(), ownPolicy, new byte[]{}), BigInteger.ZERO)) { matches = matches + 1; stateIndex = position; }
            position = position + 1;
        }
        ...
        if (!AccountLib.normalState(state, deploymentDomain, stateValidatorHash)
                ...
                || !state.recoveryNotBefore().equals(BigInteger.ZERO) || !AccountLib.stateOutput(output, state, stateValidatorHash)) return false;
        var module = AccountLib.script(state.authModule().scriptHash());
        var purpose = new ScriptPurpose.Rewarding(module);
        if (!ctx.txInfo().withdrawals().containsKey(module) || !ctx.txInfo().redeemers().containsKey(purpose)) return false;
