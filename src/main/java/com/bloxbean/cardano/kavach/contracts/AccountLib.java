...
public class AccountLib {
     ...
     */
    public static boolean lessThan(BigInteger left, BigInteger right) { return left.compareTo(right) < 0; }
    /**
     ...
     */
    public static boolean atMost(BigInteger left, BigInteger right) { return left.compareTo(right) <= 0; }
    /**
     ...
     */
    public static boolean equalInteger(BigInteger left, BigInteger right) { return left.equals(right); }
    /**
     ...
     */
    public static Credential script(byte[] hash) { return new Credential.ScriptCredential(new ScriptHash(hash)); }
    /**
    ...
    public static boolean stateOutput(TxOut output, AccountState state, byte[] stateHash) {
        boolean datumMatches = switch (output.datum()) {
            case OutputDatum.OutputDatumInline inline -> Builtins.equalsData(inline.datum(), (PlutusData)(Object)state);
            default -> false;
        };
    ...
    public static boolean authenticateFrom(AccountState state, IntentDomain domain, DeploymentDomain deployment,
        ...
        for (var candidate : candidates) {
            if (ValuesLib.containsPolicy(candidate.resolved().value(), state.accountId().policy())) {
                matches = matches + 1;
                if (!candidate.outRef().equals(domain.stateRef()) || !stateOutput(candidate.resolved(), state, stateHash)) valid = false;
            }
        }
    ...
    public static boolean validity(Validity window, ScriptContext ctx) {
        BigInteger start = window.notBefore(); BigInteger end = window.expiresAt();
     ...
     */
    public static byte[] digest(IntentEnvelope intent) { return Builtins.blake2b_256(Builtins.serialiseData((PlutusData)(Object)intent)); }
    /**
    ...
    public static boolean receipts(JulcList<RewardReceipt> table, ScriptContext ctx) {
        ...
        int positives = 0; boolean valid = true;
        ...
        for (var receipt : table) {
            ...
            if (Builtins.lengthOfByteString(hash) != 28 || !Builtins.lessThanByteString(previous, hash)
                    ...
                    || !ctx.txInfo().withdrawals().containsKey(receipt.rewardCredential()) || !index(receipt.outputIndex(), ctx)) valid = false;
            else {
    ...
    public static boolean ownSink(Credential own, Address sink, JulcList<RewardReceipt> table, ScriptContext ctx) {
        boolean keySink = switch (sink.credential()) { case Credential.PubKeyCredential key -> true; default -> false; };
        ...
        for (var receipt : table) {
            if (receipt.rewardCredential().equals(own) && (!index(receipt.outputIndex(), ctx)
                    || !ctx.txInfo().outputs().toArray().get(receipt.outputIndex().longValue()).address().equals(sink))) valid = false;
        }
        return valid;
    }
    /**
    ...
    public static boolean registration(ScriptContext ctx) {
        return switch (ctx.scriptInfo()) {
            case ScriptInfo.CertifyingScript certifying -> switch (certifying.cert()) {
                case TxCert.RegStaking registration -> registration.deposit().isPresent() && AccountLib.atMost(BigInteger.ZERO, registration.deposit().get())
                        && switch (registration.credential()) { case Credential.ScriptCredential script -> true; default -> false; };
                default -> false;
            };
            default -> false;
        };
