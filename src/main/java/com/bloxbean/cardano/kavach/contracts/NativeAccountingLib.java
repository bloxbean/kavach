...
public class NativeAccountingLib {
    ...
    public static BigInteger onlyLovelace(Value value) {
        ...
        if (Builtins.nullList(policies) || !Builtins.nullList(Builtins.tailList(policies))
                || !Builtins.equalsData(Builtins.fstPair(Builtins.headList(policies)), Builtins.bData(new byte[]{})))
        ...
        if (Builtins.nullList(tokens) || !Builtins.nullList(Builtins.tailList(tokens))
                || !Builtins.equalsData(Builtins.fstPair(Builtins.headList(tokens)), Builtins.bData(new byte[]{})))
            return BigInteger.valueOf(-1);
    ...
    static int nativeCount(Value value) {
        ...
        if (AccountLib.atMost(BigInteger.ZERO, ada))
            return AccountLib.lessThan(BigInteger.ZERO, ada) && AccountLib.uint63(ada) ? 0 : -1;
        ...
        for (byte[] policy : policies.keys()) {
            ...
            for (byte[] name : tokens.keys()) {
                ...
                if (Builtins.lengthOfByteString(policy) == 0) {
                    ...
                    count = count + 1;
                    if (Builtins.lengthOfByteString(policy) != 28 || Builtins.lengthOfByteString(name) > 32)
                        valid = false;
                }
            }
        }
    ...
    public static boolean conserve(Address accountAddress, byte[] recipientMask, BigInteger feeLimit, ScriptContext ctx) {
        ...
        PlutusData incoming = NativeValueLib.toData(zero);
        PlutusData allocated = NativeValueLib.toData(zero);
        int inputEntries = 0;
        int outputEntries = 0;
        int accountInputs = 0;
        int position = 0;
        ...
        for (var input : ctx.txInfo().inputs()) {
            if (input.resolved().address().equals(accountAddress)) {
                ...
                inputEntries = inputEntries + count;
        ...
        var inputNative = NativeValueLib.fromData(incoming);
        var outputNative = NativeValueLib.fromData(allocated);
