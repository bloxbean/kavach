...
public class PeriodicBudgetLib {
     ...
     */
    public static Usage initial() {
        return new Usage(BigInteger.ONE, BigInteger.ZERO, BigInteger.ZERO, BigInteger.ZERO);
    }
    ...
    public static boolean isProfile(PlutusData data) {
        return AccountLib.shape(data, 0, 3);
    }
    ...
    public static boolean configuration(Configuration configuration) {
        if (!isProfile((PlutusData) (Object) configuration) || !configuration.schemaVersion().equals(BigInteger.ONE)
                || Builtins.lengthOfByteString(Builtins.serialiseData((PlutusData) (Object) configuration)) > 1024)
            return false;
    ...
    public static BigInteger window(BigInteger timestamp, BigInteger period) {
        ...
        if (!AccountLib.uint63(timestamp) || timestamp.compareTo(anchor) < 0)
            return (BigInteger) (Object) Builtins.error();
    ...
    public static Usage successor(Usage previous, Budget budget, BigInteger debit, ScriptContext ctx) {
        if (!usageShape(previous) || !budgetShape(budget) || !AccountLib.uint63(debit))
            return (Usage) (Object) Builtins.error();
        ...
        if (!AccountLib.uint63(upper) || lower.compareTo(upper) >= 0 || previous.windowStart().compareTo(lower) > 0
                || upper.compareTo(end) > 0 || (upper.equals(end) && range.to().isInclusive()))
            return (Usage) (Object) Builtins.error();
