...
/**
 * Initial raw-Ed25519 authorization module. Independently authenticates canonical state,
 ...
 * <p>Immutable parameters, applied from left to right: {@code moduleVersion, abiVersion, deploymentDomain, stateValidatorHash, coreCheckpointHash, rewardSink}.</p>
...
public class Ed25519Module {
    ...
    static boolean authorize(ModuleRedeemer invocation, ScriptContext ctx) {
        ...
        return Ed25519LifecycleLib.current(invocation, state);
    }
    ...
    static boolean initialize(GenesisModuleRedeemer genesis, ScriptContext ctx) {
        ...
        var own = AccountLib.script(state.authModule().scriptHash());
        boolean purpose = switch (ctx.scriptInfo()) { case ScriptInfo.RewardingScript reward -> reward.credential().equals(own); default -> false; };
        if (!purpose || !AccountLib.receipts(genesis.receipts(), ctx) || !AccountLib.ownSink(own, rewardSink, genesis.receipts(), ctx)
                ...
                || !ValuesLib.assetOf(ctx.txInfo().mint(), state.accountId().policy(), new byte[]{}).equals(BigInteger.ONE)) return false;
        int matches = 0; boolean valid = true; int position = 0;
        for (var output : ctx.txInfo().outputs()) {
            if (!AccountLib.equalInteger(ValuesLib.assetOf(output.value(), state.accountId().policy(), new byte[]{}), BigInteger.ZERO)) {
                matches = matches + 1;
                if (!AccountLib.stateOutput(output, state, stateValidatorHash) || AccountLib.receiptAt(genesis.receipts(), BigInteger.valueOf(position))) valid = false;
            }
            position = position + 1;
        }
        var config = (Ed25519Config)(Object)state.authConfig();
        if (!valid || matches != 1 || !Ed25519Lib.configuration(config) || AccountLib.listSize((JulcList<PlutusData>)(Object)genesis.configPossession()) != AccountLib.listSize((JulcList<PlutusData>)(Object)config.keys())) return false;
        ...
        return Ed25519Lib.signatures(genesis.configPossession(), config.keys(), Builtins.blake2b_256(Builtins.serialiseData((PlutusData)(Object)envelope)), 16);
    }
        ...
        return switch (invocation.intent().action()) {
            case ReplaceModule replacement -> {
                ...
                yield AccountLib.moduleShape(replacement.newModule()) && !candidate.equals(oldCredential)
                        ...
                        && (own.equals(oldCredential) ? Ed25519LifecycleLib.current(invocation, previous)
                            : own.equals(candidate) && Ed25519LifecycleLib.candidate(invocation, replacement));
            }
            default -> ctx.txInfo().withdrawals().size() == 2 && own.equals(oldCredential)
                    && Ed25519LifecycleLib.current(invocation, previous);
        };
    }
