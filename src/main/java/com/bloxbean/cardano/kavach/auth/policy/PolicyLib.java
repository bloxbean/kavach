...
public class PolicyLib {
    ...
    public static boolean configuration(PolicyConfig config) {
        ...
        if (AccountLib.lessThan(roles.spend().threshold().subtract(BigInteger.valueOf(extra)), config.smallSpend().threshold()))
            return false;
    ...
    public static ThresholdPolicy spendPolicy(PolicyConfig config, Action action) {
        return switch (action) {
            case Spend spend -> {
                ...
                boolean adaOnly = AccountLib.atMost(BigInteger.ZERO, debit);
