...
public class BrowserEvidenceLib {
    ...
    public static boolean cose(byte[] key, byte[] message, byte[] packed, BigInteger network) {
        ...
        if ((size != 93 && size != 121 && !withKid) || Builtins.lengthOfByteString(key) != 32
                || Builtins.lengthOfByteString(message) != 32 || (network.intValue() != 0 && network.intValue() != 1)) return false;
        ...
        long kind = header / 16;
        if (header % 16 != network.longValue() || !(addressLength == 29 ? kind == 6 : kind == 0 || kind == 2)) return false;
        if (!Builtins.equalsByteString(Builtins.sliceByteString(1, 28, packed), Builtins.blake2b_224(key))) return false;
