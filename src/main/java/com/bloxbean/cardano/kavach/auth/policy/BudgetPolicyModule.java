...
/**
 * Budget-aware mixed browser authorization candidate (scheme 4). Each configured credential is fixed
 ...
 * coreCheckpointHash, rewardSink. The budget-capable asset validator enforces the core envelope independently.
 */
@MultiValidator
public class BudgetPolicyModule {
    @Param
    ...
    static boolean authorize(ModuleRedeemer invocation, ScriptContext ctx) {
        ...
        return BudgetPolicyLifecycleLib.current(invocation, state, deploymentDomain.networkId(), ctx);
    }
        ...
        return switch (invocation.intent().action()) {
            case ReplaceModule replacement -> {
                ...
                yield AccountLib.moduleShape(replacement.newModule()) && !candidate.equals(oldCredential)
                        ...
                        && (own.equals(oldCredential) ? BudgetPolicyLifecycleLib.current(invocation, previous, deploymentDomain.networkId(), ctx)
                        : own.equals(candidate) && BudgetPolicyLifecycleLib.candidate(invocation, replacement, deploymentDomain.networkId(), ctx));
            }
            default -> ctx.txInfo().withdrawals().size() == 2 && own.equals(oldCredential)
                    && BudgetPolicyLifecycleLib.current(invocation, previous, deploymentDomain.networkId(), ctx);
        };
    }
