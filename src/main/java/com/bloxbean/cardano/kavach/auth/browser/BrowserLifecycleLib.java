...
/**
 * Browser-module administration and recovery proof rules. Old-role authority cannot be
 ...
 */
@OnchainLibrary
public class BrowserLifecycleLib {
    /**
     ...
     */
    public static boolean approval(Optional<Proof> supplied, Ed25519Config current, ThresholdPolicy selected, IntentEnvelope request, BigInteger mode, BigInteger network, ScriptContext ctx) {
        if (supplied.isEmpty()) return false;
        var proof = supplied.get();
        return Builtins.equalsData((PlutusData)(Object)supplied,
                (PlutusData)(Object)Optional.of(new Proof(mode, proof.signatures())))
                && BrowserEvidenceLib.signatures(proof.signatures(), current.keys(), AccountLib.digest(request), 8, mode, network, ctx)
    ...
    public static boolean introduced(JulcList<Signature> evidence, JulcList<KeyEntry> oldKeys,
            Ed25519Config destination, byte[] message, boolean targetThreshold, BigInteger mode, BigInteger network, ScriptContext ctx) {
        if (!evidence.isEmpty() && !BrowserEvidenceLib.signatures(evidence, destination.keys(), message, 16, mode, network, ctx)) return false;
     ...
     */
    public static boolean candidate(ModuleRedeemer invocation, ReplaceModule replacement, BigInteger mode, BigInteger network, ScriptContext ctx) {
        ...
        return Ed25519Lib.configuration(config)
                ...
                && BrowserEvidenceLib.signatures(invocation.configPossession(), config.keys(),
                    configMessage(invocation.intent(), replacement.newModule(), replacement.newConfig()), 16, mode, network, ctx);
    }
     ...
     */
    public static boolean current(ModuleRedeemer invocation, AccountState previous, BigInteger mode, BigInteger network, ScriptContext ctx) {
        ...
        if (!completing && !roleApproval(invocation, config, mode, network, ctx)) return false;
            ...
            case ReplaceConfig replacement -> {
                ...
                yield Ed25519Lib.configuration(target)
                        && introduced(invocation.configPossession(), config.keys(), target,
                            configMessage(invocation.intent(), previous.authModule(), replacement.newConfig()), false, mode, network, ctx);
            }
                ...
                yield absent && switch (previous.mode()) {
                    case RecoveryPending pending -> {
                        ...
                        yield completion.currentSequence().equals(previous.recoverySequence())
                                ...
                                && introduced(invocation.configPossession(), config.keys(), target,
                                    Builtins.blake2b_256(Builtins.serialiseData((PlutusData)(Object)preimage)), true, mode, network, ctx);
                    }
                    default -> false;
                };
            }
    ...
    static boolean roleApproval(ModuleRedeemer invocation, Ed25519Config currentConfig, BigInteger mode, BigInteger network, ScriptContext ctx) {
        ...
        return approval(invocation.operationProof(), currentConfig, selected, invocation.intent(), mode, network, ctx);
    }
}
