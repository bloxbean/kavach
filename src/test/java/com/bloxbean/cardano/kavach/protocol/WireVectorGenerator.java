...
public final class WireVectorGenerator {
    public static void main(String[] args) throws Exception {
        ...
        for (int tag = 0; tag < 9; tag++) {
            ...
            var signer = Signature.getInstance("Ed25519"); signer.initSign(key.getPrivate()); signer.update(digest);
            var vector = new LinkedHashMap<String, Object>();
            vector.put("actionTag", tag); vector.put("cbor", HexFormat.of().formatHex(cbor));
            vector.put("digest", HexFormat.of().formatHex(digest)); vector.put("publicKey", HexFormat.of().formatHex(pub));
            vector.put("signature", HexFormat.of().formatHex(signer.sign())); vector.put("rendering", WireFormat.renderIntent(envelope, tag == 8 ? WireFixtures.wholeValue() : null));
            if (tag == 8) vector.put("resolvedInputValueCbor", PlutusDataAdapter.toClientLib(WireFixtures.wholeValue()).serializeToHex());
            vectors.add(vector);
        }
        ...
        for (var entry : domains.entrySet()) proofVectors.put(entry.getKey(), Map.of(
        ...
        var setupEnvelope = ProofDomains.genesis(setupState); byte[] setupDigest = WireFormat.digest(setupEnvelope);
        var proofs = new ArrayList<PlutusData>();
        for (int i = 0; i < setupKeys.size(); i++) {
            var signer = Signature.getInstance("Ed25519"); signer.initSign(setupKeys.get(i).getPrivate()); signer.update(setupDigest);
    ...
    private static Object edge(String name, PlutusData envelope, PlutusData resolved, KeyPair key) throws Exception {
        ...
        vector.put("name", name); vector.put("cbor", HexFormat.of().formatHex(cbor));
        ...
        if (resolved != null) vector.put("resolvedInputValueCbor",PlutusDataAdapter.toClientLib(resolved).serializeToHex());
        return vector;
    }
