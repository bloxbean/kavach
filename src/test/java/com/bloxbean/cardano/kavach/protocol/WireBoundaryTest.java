...
class WireBoundaryTest {
    ...
    @Test
    void fullRegistryAndRoleSizesFitConfigStateAndIntentCaps() throws Exception {
        ...
        WireFormat.validateState(replace(state(), 6, config));
    ...
    @Test
    void maximumCountsAndOneOverAreUnambiguous() {
        var inputs = new ArrayList<PlutusData>();
        var recipients = new ArrayList<PlutusData>();
        for (int i = 0; i < 8; i++) {
            inputs.add(input(i + 1));
            recipients.add(rec(number(i), address(), value()));
        }
        ...
        var assets = new ArrayList<PlutusData>();
        assets.add(rec(bytes(0, 0), bytes(0, 0), number(2_000_000)));
