...
class AccountCodecConformanceTest {
    ...
    private TxOutRef ref(int index) {
        return new TxOutRef(new TxId(bytes(32, 6)), n(index));
    }
    ...
    private Action action(int tag) {
        return switch (tag) {
            ...
            case 8 ->
                    new TransferWholeUtxo(ref(1), n(0), recipient, WireFormat.ledgerValueDigest(WireFixtures.wholeValue()));
            default -> throw new AssertionError(tag);
        };
    }

    @Test
    void allActionTagsMatchFrozenBytesDigestsAndRendering() throws Exception {
    ...
    @Test
    void completeStateAndGenesisDomainMatchFrozenFixtures() throws Exception {
    ...
    @Test
    void encodingIsExplicitAndDoesNotAliasByteArrayInputs() {
        byte[] value = {1, 2, 3};
        var encoded = AccountCodec.data(value);
        value[0] = 9;
