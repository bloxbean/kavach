...
class RecoveryModelTest {
    ...
    private State initial() {
        return new State(BigInteger.ZERO, BigInteger.ZERO, BigInteger.ZERO, MIN_DELAY, MIN_COOLDOWN, Mode.NORMAL, null, null);
    }

    private Interval at(BigInteger t) {
        return new Interval(t, t.add(BigInteger.ONE));
    }

    @Test
    void delayedCompletionUsesUpperForDeadlineAndLowerForEligibility() {
        ...
        assertEquals(Mode.NORMAL, complete.mode());
        assertEquals(BigInteger.ONE, complete.sequence());
        assertEquals(BigInteger.TWO, complete.version());
        assertEquals(pending.notBefore(), complete.notBefore());
    }

    @Test
    void cancelFreezeUnfreezePreservesCooldownAndDefensiveWindow() {
    ...
    @Test
    void pendingRejectsSpendUnfreezeAdminAndTargetSubstitution() {
        var pending = apply(initial(), Action.START, Authority.RECOVERY, at(BigInteger.ZERO), COMMITMENT);
    ...
    @Test
    void allWrongRoleCombinationsReject() {
        for (var action : Action.values())
            for (var authority : Authority.values()) {
                ...
                if (!right)
                    assertThrows(IllegalArgumentException.class, () -> apply(initial(), action, authority, at(BigInteger.ZERO), COMMITMENT));
            }
    }

    @Test
    void arithmeticAndIntervalBoundsFailClosed() {
    ...
    @Test
    void unsignedTimingChangesRejectEvenWhenTheyIncreaseSecurityDelays() {
        var old = initial();
    ...
    @Test
    void deterministicRandomTransitionTracesPreserveMonotonicity() {
