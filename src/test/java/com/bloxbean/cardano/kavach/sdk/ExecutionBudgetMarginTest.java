...
class ExecutionBudgetMarginTest {
    private static ProtocolParams limits(long memory, long cpu) {
        ...
        params.setMaxTxExMem(Long.toString(memory));
        params.setMaxTxExSteps(Long.toString(cpu));
        return params;
    }
    ...
    private static Result<List<EvaluationResult>> result(List<EvaluationResult> values) {
        return Result.success("fixture").withValue(values);
    }
    ...
    private static byte[] transaction(int count) throws Exception {
        ...
        for (int i = 0; i < count; i++)
            redeemers.add(new Redeemer(RedeemerTag.Reward, BigInteger.valueOf(i), BigIntPlutusData.of(0), new ExUnits(BigInteger.ONE, BigInteger.ONE)));
        tx.getWitnessSet().setRedeemers(redeemers);
        return tx.serialize();
    }

    @Test
    void roundsEachRedeemerUpAndDoesNotModifyCachedEstimates() throws Exception {
    ...
    @Test
    void exactPaddedLimitPassesButCombinedExcessRejectsWithoutClipping() throws Exception {
    ...
    @Test
    void rejectsMissingDuplicateUnexpectedAndNegativeEstimates() throws Exception {
    ...
    @Test
    void measuredPositiveReceiptScenarioFitsFivePercentButNotTen() throws Exception {
    ...
    @Test
    void passesUnderlyingRejectionThroughAndNeverCallsItSuccess() throws Exception {
    ...
    @Test
    void refusesInvalidAllowanceAndLimits() {
        TransactionEvaluator evaluator = (cbor, inputs) -> result(List.of());
        for (int percent : new int[]{0, -1, 101})
            assertThrows(IllegalArgumentException.class, () -> new ExecutionBudgetMargin(evaluator, limits(1, 1), percent));
