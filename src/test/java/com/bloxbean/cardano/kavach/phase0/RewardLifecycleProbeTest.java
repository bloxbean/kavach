...
class RewardLifecycleProbeTest {
    ...
    @Test void scalarRedeemersMatchCanonicalCborVectors() {
    ...
    @Test void acceptsExplicitRegistrationWithoutAuthoritySignature() {
    ...
    @Test void acceptsZeroWithoutReceipt() {
        assertInstanceOf(EvalResult.Success.class, eval(reward(0), -1));
    }
    @Test void acceptsFullPositiveReceiptAndExternalMinAdaTopup() {
    ...
    @ParameterizedTest @ValueSource(strings = {"missing", "wrong-credential", "no-signer", "negative", "underpay", "no-output", "bad-index", "negative-index", "extra-withdrawal", "wrong-address", "stake-credential", "token", "datum", "reference-script", "datum-hash", "zero-index", "huge-index"})
    void rejectsUnsafeWithdrawal(String mutation) {
        ...
        switch (mutation) {
            ...
            case "wrong-credential" -> ctx = ScriptContextTestBuilder.rewarding(script).withdrawal(key, BigInteger.ONE).signer(authority);
            case "no-signer" -> ctx = ScriptContextTestBuilder.rewarding(script).withdrawal(script, BigInteger.valueOf(4_000_000));
            case "negative" -> ctx = reward(-1);
            case "underpay" -> out = receipt(3_999_999);
            case "no-output" -> { rejects(ctx, 0); return; }
            ...
            case "wrong-address" -> out = new TxOut(new Address(script, Optional.empty()), out.value(), out.datum(), Optional.empty());
            case "stake-credential" -> out = new TxOut(new Address(key, Optional.of(new StakingCredential.StakingHash(key))), out.value(), out.datum(), Optional.empty());
            case "token" -> out = new TxOut(sink, out.value().merge(Value.singleton(new PolicyId(new byte[28]), TokenName.EMPTY, BigInteger.ONE)), out.datum(), Optional.empty());
            case "datum-hash" -> out = new TxOut(sink, out.value(), new OutputDatum.OutputDatumHash(new DatumHash(new byte[32])), Optional.empty());
            case "datum" -> out = new TxOut(sink, out.value(), new OutputDatum.OutputDatumInline(PlutusData.integer(0)), Optional.empty());
            case "reference-script" -> out = new TxOut(sink, out.value(), out.datum(), Optional.of(new ScriptHash(new byte[28])));
            case "zero-index" -> ctx = reward(0);
            default -> throw new AssertionError(mutation);
        }
        rejects(ctx.output(out), index);
    }
    @ParameterizedTest @ValueSource(strings = {"legacy", "wrong-deposit", "key-credential", "deregister", "delegate", "register-delegate", "wrong-redeemer"})
    void rejectsUnsupportedCertificate(String mutation) {
        ...
        switch (mutation) {
            ...
            case "delegate" -> cert = new TxCert.DelegStaking(script, new Delegatee.Stake(new PubKeyHash(new byte[28])));
            case "register-delegate" -> cert = new TxCert.RegDeleg(script, new Delegatee.Stake(new PubKeyHash(new byte[28])), deposit);
            case "wrong-redeemer" -> index = 0;
            default -> throw new AssertionError(mutation);
        }
        rejects(ScriptContextTestBuilder.certifying(BigInteger.ZERO, cert), index);
    }
    @Test void rejectsOtherPurposesThroughCompiledWrapper() {
