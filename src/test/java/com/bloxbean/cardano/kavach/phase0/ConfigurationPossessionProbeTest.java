...
class ConfigurationPossessionProbeTest {
    ...
    void maximumKeysProofsAndEnvelopeFitMeasuredBudget() throws Exception {
        ...
        } while (Builtins.serialiseData(envelope).length > 1536);
        ...
        evidence.put("scope", "Configuration possession primitive maximum input-size case; opaque payload, not a valid full account operation");
        evidence.put("keyCount", 16);
        evidence.put("signatureCount", 16);
        ...
        Files.writeString(Path.of("build/phase0/possession-budget.json"), JsonUtil.getPrettyJson(evidence));
    ...
    void rejectsInvalidConfigOrEvidence(String mutation) throws Exception {
        ...
        switch (mutation) {
            ...
            case "out-of-range" ->
                    proofs.set(15, PlutusData.constr(0, PlutusData.integer(16), ((PlutusData.ConstrData) proofs.get(15)).fields().get(1)));
            case "too-few" -> proofs.removeLast();
            case "bad-last-signature" ->
                    proofs.set(15, PlutusData.constr(0, PlutusData.integer(15), PlutusData.bytes(new byte[64])));
            case "extra-proof-field" ->
                    proofs.set(15, PlutusData.constr(0, PlutusData.integer(15), ((PlutusData.ConstrData) proofs.get(15)).fields().get(1), PlutusData.integer(0)));
            case "wrong-proof-tag" ->
                    proofs.set(15, new PlutusData.ConstrData(1, ((PlutusData.ConstrData) proofs.get(15)).fields()));
        }
