...
class ValidityWindowProbeTest {
    ...
    @Test void exactHalfOpenContainmentAndDeadlineSucceed() {
    ...
    @ParameterizedTest @ValueSource(strings={"lower-outside","upper-outside","closed-upper","infinite-lower","infinite-upper","empty","zero-width","wide","negative","deadline","no-authority"})
    void rejectsUnsafeIntervals(String mutation) {
        Interval range = interval(1000,2000,false); long lower=1000, upper=2000, deadline=1000;
        switch (mutation) {
