...
class BindingCheckpointDevkitTest {
    ...
    @Test void pairedRewardingExecutionAndMismatchedEvidence() throws Exception {
        ...
        topUp(sponsor.baseAddress(), 100); topUp(sponsor.baseAddress(), 10); topUp(creator.baseAddress(), 20);
        ...
        evidence.put("mintTx", mintId); evidence.put("registrationTx", registrationId); evidence.put("pairedWithdrawalTx", validId);
        ...
        evidence.put("coreCbor", pair.core().getCborHex()); evidence.put("moduleCbor", pair.module().getCborHex());
        evidence.put("policy", policyId); evidence.put("publicKey", HexUtil.encodeHexString(ProbeFixtures.publicKey(key)));
        evidence.put("authorizationCbor", PlutusDataAdapter.toClientLib(auth).serializeToHex());
        evidence.put("redeemers", valid.getWitnessSet().getRedeemers()); evidence.put("transactionBytes", valid.serialize().length);
        evidence.put("protocolParameters", params.getValue()); evidence.put("remoteEvaluation", remoteEvaluation.toString());
        evidence.put("invalidSignatureRejection", invalidResult.toString()); evidence.put("mismatchedRedeemerRejection", mismatchResult.toString());
