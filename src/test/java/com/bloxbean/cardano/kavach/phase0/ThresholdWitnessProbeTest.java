...
class ThresholdWitnessProbeTest {
    ...
    @Test void maximumKeysProofsAndEnvelopeFitMeasuredBudget() throws Exception {
        var keys = keys(8); int size = 8192; PlutusData envelope;
        do { envelope = PlutusData.bytes(new byte[size--]); } while (Builtins.serialiseData(envelope).length > 8192);
        ...
        var evidence = new LinkedHashMap<String, Object>(); evidence.put("scope", "Threshold primitive maximum input-size case; opaque payload, not a valid full account operation");
        evidence.put("keyCount", 8); evidence.put("signatureCount", 8); evidence.put("serializedEnvelopeBytes", Builtins.serialiseData(envelope).length);
        evidence.put("cpu", success.consumed().cpuSteps()); evidence.put("memory", success.consumed().memoryUnits());
        Files.createDirectories(Path.of("build/phase0")); Files.writeString(Path.of("build/phase0/threshold-budget.json"), JsonUtil.getPrettyJson(evidence));
    ...
    void rejectsInvalidConfigOrEvidence(String mutation) throws Exception {
        ...
        var proofs = proofs(keys, envelope); int threshold = 8;
        switch (mutation) {
            ...
            case "out-of-range" -> proofs.set(7, PlutusData.constr(0, PlutusData.integer(8), ((PlutusData.ConstrData) proofs.get(7)).fields().get(1)));
            case "too-few" -> proofs.removeLast();
            case "bad-last-signature" -> proofs.set(7, PlutusData.constr(0, PlutusData.integer(7), PlutusData.bytes(new byte[64])));
            case "extra-proof-field" -> proofs.set(7, PlutusData.constr(0, PlutusData.integer(7), ((PlutusData.ConstrData) proofs.get(7)).fields().get(1), PlutusData.integer(0)));
            case "wrong-proof-tag" -> proofs.set(7, new PlutusData.ConstrData(1, ((PlutusData.ConstrData) proofs.get(7)).fields()));
        }
