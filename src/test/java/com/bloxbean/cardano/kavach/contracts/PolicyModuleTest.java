...
class PolicyModuleTest {
    ...
    void fixedMethodsAndAmountTier(String scenario) throws Exception {
        ...
        if (scenario.equals("swapped-method")) signatures = AccountFixtures.list(witness, new Signature(BigInteger.ONE, new byte[0]));
        ...
        if (scenario.equals("swapped-method")) ctx.signer(BrowserSignatures.keyHash(AccountFixtures.publicKey(f.keys.get(1))));
