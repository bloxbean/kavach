...
class AccountReceiptsTest {
    ...
    JulcList<RewardReceipt> receipts(int coreIndex, int moduleIndex) throws Exception {
        var a = new RewardReceipt(f.core, BigInteger.valueOf(coreIndex)); var b = new RewardReceipt(f.module, BigInteger.valueOf(moduleIndex));
    ...
    ScriptContextTestBuilder context(String role, int coreIndex, int moduleIndex, long amount) throws Exception {
        var intent = f.spend(0); var original = f.authorization(intent);
    ...
    @Test void positiveRewardsWithIndependentReceiptsAndTopupsPassAllScripts() throws Exception {
        for (var role : List.of("core", "module", "asset")) assertInstanceOf(EvalResult.Success.class, f.evaluate(role, context(role, 2, 3, 1)));
    }
    @Test void receiptReuseUnderpaymentAndZeroReceiptReject() throws Exception {
    ...
    @Test void distinctRegistrationBranchCannotDeregisterOrAuthorize() {
        for (var role : List.of("core", "module")) {
