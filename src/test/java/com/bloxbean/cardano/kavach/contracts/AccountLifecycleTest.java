...
class AccountLifecycleTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"config", "freeze", "unfreeze", "start-normal", "start-frozen", "cancel", "complete"})
    void fullCurrentModuleLifecycle(String operation) throws Exception {
        long lower = 1000, upper = 9999;
        var old = f.state;
        Action action = new Freeze();
        AccountState next = state(old, 1, 0, 0, new Frozen());
        int[] signers = {1};
        switch (operation) {
            case "config" -> {
                action = new ReplaceConfig(old.authConfig());
                next = state(old, 1, 0, 0, new Normal());
                signers = new int[]{0, 1};
            }
            case "unfreeze" -> {
                old = state(old, 1, 0, 0, new Frozen());
                action = new Unfreeze();
                next = state(old, 2, 0, 0, new Normal());
                signers = new int[]{2};
            }
            ...
            case "cancel", "complete" -> {
                ...
                old = state(old, 1, 1, 3600000, new RecoveryPending(commitment, BigInteger.valueOf(86400000), old.authConfig()));
                if (operation.equals("cancel")) {
                    action = new CancelRecovery(BigInteger.ONE, commitment);
                    next = state(old, 2, 1, upper + 3600000, new Frozen());
                    signers = new int[]{2};
                } else {
                    lower = 86400000;
                    upper = 86409999;
                    action = new CompleteRecovery(BigInteger.ONE, old.authConfig());
                    next = state(old, 2, 1, 3600000, new Normal());
                }
            }
        }
        var request = intent(old, action, lower, upper);
        var auth = approval(old, request, signers);
        ...
        assertTrue(memory <= 16500000, "combined memory " + memory);
        assertTrue(cpu <= 10000000000L, "combined cpu " + cpu);
    }

    @ParameterizedTest
    @ValueSource(strings = {"everyday-freeze", "guardian-unfreeze", "guardian-cancel", "old-spend-completion"})
    void wrongAuthoritiesRejectAtModule(String attack) throws Exception {
        var old = f.state;
        Action action = new Freeze();
        var next = state(old, 1, 0, 0, new Frozen());
        int signer = 0;
        long lower = 1000, upper = 9999;
        ...
        var request = intent(old, action, lower, upper);
        var auth = approval(old, request, signer);
    ...
    @ParameterizedTest
    @ValueSource(strings = {"version", "sequence", "delay", "cooldown", "deadline", "mode", "deposit", "nftless", "core-binding"})
    void authorizedFreezeCannotChangeForbiddenStateFields(String attack) throws Exception {
        var old = f.state;
        var next = state(old, 1, 0, 0, new Frozen());
        var request = intent(old, new Freeze(), 1000, 9999);
        var auth = approval(old, request, 1);
        var ctx = context("state", old, next, auth, 1000, 9999).buildPlutusData();
        ctx = switch (attack) {
            ...
            case "nftless" ->
                    AccountAdversarialTest.field(ctx, Value.lovelace(BigInteger.valueOf(10000000)).toPlutusData(), 0, 0, 0, 1, 1);
            case "core-binding" ->
                    AccountAdversarialTest.field(ctx, PlutusData.bytes(new byte[28]), 0, 2, 0, 2, 0, 3, 2);
