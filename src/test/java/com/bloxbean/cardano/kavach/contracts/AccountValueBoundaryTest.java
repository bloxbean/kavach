...
class AccountValueBoundaryTest {
    ...
    @Test
    void completeNativeMapPreservesSmallAndMaximumPartialQuantities() throws Exception {
    ...
    @Test
    void partialQuantityOverflowLossAndCreationReject() throws Exception {
    ...
    @ParameterizedTest
    @ValueSource(longs = {0, 1, 200000, 5000000})
    ...
    @Test
    void signedFeeCannotExceedProtocolCeilingOrBeNegative() throws Exception {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"aggregate-overflow", "asset-union"})
    void aggregateBoundsRejectEvenWhenEachInputAndOutputFits(String attack) throws Exception {
        ...
        a = a.merge(Value.lovelace(BigInteger.valueOf(-1000000)));
        b = b.merge(Value.lovelace(BigInteger.valueOf(-1000000)));
    ...
    void signedRecipientMaskCoversBothBytesAndHighestOutputIndex(int index) throws Exception {
        ...
        int changeIndex = index == 0 ? 1 : 0;
        for (int i = 0; i < 16; i++)
            outputs.add(AccountFixtures.output(i == changeIndex ? f.accountAddress : f.sink, i == changeIndex ? 8000000 : 2000000).toPlutusData());
