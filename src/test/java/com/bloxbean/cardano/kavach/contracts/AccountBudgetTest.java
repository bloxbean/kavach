...
class AccountBudgetTest {
    ...
    void maximumInputOutputCountsAndEightSignaturesFitTogether(boolean positiveRewards, int recipientCount) throws Exception {
        ...
        var keys = new ArrayList<KeyPair>(); var registry = new ArrayList<KeyEntry>();
        for (int i = 0; i < 16; i++) {
        ...
        var refs = new ArrayList<TxOutRef>(); var inputs = new ArrayList<PlutusData>(); var outputs = new ArrayList<PlutusData>();
        ...
        for (int i = 0; i < 8; i++) inputs.add(new TxInInfo(AccountFixtures.ref(100 + i), AccountFixtures.output(f.sink, 2000000)).toPlutusData());
        ...
        for (int i = 0; i < changeCount; i++) {
            var assets = new ArrayList<Integer>();
            if (recipientCount == 0) { if (i < 12) assets.add(i % 11); }
            else if (i >= recipientCount) for (int j = 0; j < 12; j++) if (j % (changeCount - recipientCount) == i - recipientCount) assets.add(j % 11);
        ...
        var proofs = new ArrayList<Signature>(); for (int i = 0; i < 8; i++) proofs.add(AccountFixtures.sign(i, keys.get(i), digest));
        ...
        long memory = 0; long cpu = 0; long allocatedMemory = 0; long allocatedCpu = 0; var budgets = new LinkedHashMap<String, Object>();
        for (int execution = 0; execution < 10; execution++) {
            ...
            for (int signer = 0; signer < 16; signer++) {
                byte[] hash = new byte[28]; hash[27] = (byte) signer;
            ...
            for (var ref : refs) redeemers = redeemers.insert(new ScriptPurpose.Spending(ref), AccountCodec.data(new AssetRedeemer(BigInteger.ONE, digest)));
            var references = new ArrayList<PlutusData>(); references.add(f.stateInput().toPlutusData());
            for (int i = 0; i < 3; i++) references.add(new TxInInfo(AccountFixtures.ref(200 + i), AccountFixtures.output(f.sink, 1)).toPlutusData());
            ...
            if (execution < 8) context = AccountAdversarialTest.field(context, new ScriptInfo.SpendingScript(refs.get(execution), Optional.empty()).toPlutusData(), 2);
        ...
        budgets.put("positiveRewards", positiveRewards); budgets.put("recipientCount", recipientCount);
        budgets.put("totalMemory", memory); budgets.put("totalCpu", cpu);
        budgets.put("memoryWithFivePercentAllowance", allocatedMemory); budgets.put("cpuWithFivePercentAllowance", allocatedCpu);
        ...
        Files.createDirectories(Path.of("build/phase1")); Files.writeString(Path.of("build/phase1/combined-" + (positiveRewards ? "positive-reward-" : "") + (recipientCount == 0 ? "" : "eight-recipient-") + "budget.json"), JsonUtil.getPrettyJson(budgets));
    ...
    static Value value(long ada, List<Integer> assets) {
        ...
        for (var id : assets.stream().sorted(Comparator.reverseOrder()).toList()) {
            byte[] policy = new byte[28]; policy[0] = (byte)(id + 1);
