...
class AccountLocatorTest {
    ...
    void originalLocatorResolvesCurrentStateAcrossLifecycleAndModuleChanges(String mode) throws Exception {
        var candidate = f.moduleScript.getScriptHash().clone();
        candidate[0] ^= 1;
        ...
        for (var key : f.keys)
            assertFalse(backup.backup().contains(HexUtil.encodeHexString(AccountFixtures.publicKey(key))));
    }
    ...
    void untrustedProviderClaimsMustMatchCompleteLocatorAndCustody(String attack) throws Exception {
        ...
        switch (attack) {
            case "quantity" ->
                    input.setAmount(List.of(Amount.ada(12), Amount.asset(HexUtil.encodeHexString(f.state.accountId().policy()), BigInteger.TWO)));
            ...
            case "domain", "core" -> {
                ...
                if (attack.equals("domain"))
                    changed = AccountAdversarialTest.field(AccountCodec.data(f.state), PlutusData.bytes(HexUtil.decodeHexString("01".repeat(32))), 2, 2);
    ...
    @Test
    void backupRejectsTrailingBytesUnknownVersionAndUnboundedInput() {
