...
class AccountMutationPreparationTest {
    ...
    @Test
    void freezeAttachesConsumedStateAndBothWithdrawals() throws Exception {
    ...
    void rejectsBeforeChangingBuilder(String attack) throws Exception {
        ...
        switch (attack) {
            ...
            case "wrong-quantity" ->
                    input.setAmount(List.of(Amount.ada(10), Amount.asset(HexUtil.encodeHexString(f.state.accountId().policy()), BigInteger.TWO)));
            case "different-state" ->
                    input.setInlineDatum(PlutusDataAdapter.toClientLib(AccountCodec.data(lifecycle.state(f.state, 1, 0, 0, new Frozen()))).serializeToHex());
            case "custody" ->
                    input.setAddress(AddressProvider.getEntAddress(f.assetScript, new Network(0, 42)).toBech32());
            ...
            case "scheme" ->
                    approval = new ModuleRedeemer(BigInteger.ONE, request, Optional.of(new Proof(BigInteger.ONE,
            ...
            case "zero-receipt" ->
                    approval = new ModuleRedeemer(BigInteger.ONE, request, approval.operationProof(), JulcList.empty(),
        ...
        var supplied = approval;
        var candidateProof = candidate;
        var rewards = balances;
        var range = bounds;
        var tx = new Tx();
        var before = List.copyOf(tx.getIntentions());
