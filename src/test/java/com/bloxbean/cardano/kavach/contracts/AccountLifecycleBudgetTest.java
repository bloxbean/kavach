...
class AccountLifecycleBudgetTest {
    ...
    void mutationsFitCombinedLedgerLimitsWithAllowance(String operation) throws Exception {
        ...
        switch (operation) {
            case "config" -> {
                action = new ReplaceConfig(target);
            ...
            case "unfreeze" -> {
                old = lifecycle.state(old, 1, 0, 0, new Frozen());
                action = new Unfreeze();
                signers = new int[]{8, 9, 10, 11, 12, 13, 14, 15};
            }
            case "start" -> {
                action = new StartRecovery(BigInteger.ONE, target);
                signers = new int[]{0, 1, 2, 3, 4, 5, 6, 7};
            }
        ...
        if (operation.equals("config") || operation.equals("complete")) {
            ...
            for (int id = 0; id < 16; id++)
                signatures.add(AccountFixtures.sign(id, destination.keys.get(id), WireFormat.digest(message)));
        ...
        for (var role : List.of("state", "core", "module")) {
            ...
            for (int index = 0; index < 14; index++)
                context.input(new TxInInfo(AccountFixtures.ref(41 + index), AccountFixtures.output(f.sink, 1000000)))
                        .output(AccountFixtures.output(f.sink, index == 13 ? 1000002 : 1000000));
            for (int index = 0; index < 4; index++)
                context.referenceInput(new TxInInfo(AccountFixtures.ref(100 + index), AccountFixtures.output(f.sink, 1000000)));
