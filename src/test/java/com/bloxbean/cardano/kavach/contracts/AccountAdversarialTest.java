...
class AccountAdversarialTest {
    ...
    static PlutusData replace(PlutusData data, PlutusData before, PlutusData after) {
        ...
        return switch (data) {
            case PlutusData.ConstrData c ->
                    new PlutusData.ConstrData(c.tag(), c.fields().stream().map(d -> replace(d, before, after)).toList());
            case PlutusData.ListData l ->
                    new PlutusData.ListData(l.items().stream().map(d -> replace(d, before, after)).toList());
            case PlutusData.MapData m ->
                    new PlutusData.MapData(m.entries().stream().map(e -> new PlutusData.Pair(replace(e.key(), before, after), replace(e.value(), before, after))).toList());
            default -> data;
        };
    }

    @ParameterizedTest
    @ValueSource(strings = {"recipient-amount", "recipient-address", "change-address", "change-stake", "change-datum", "fee-theft", "unexpected-token", "missing-account-input", "extra-account-input", "other-script-input", "recipient-reuse"})
    void immutableAnchorRejectsValueAndReplayAttacks(String attack) throws Exception {
        ...
        var builder = f.context("asset", intent, auth);
        var context = builder.buildPlutusData();
        ...
        context = switch (attack) {
            case "recipient-amount" ->
                    field(context, Value.lovelace(BigInteger.valueOf(1999999)).toPlutusData(), 0, 2, 0, 1);
            ...
            case "change-stake" ->
                    field(context, new Address(f.accountAddress.credential(), keyAddress.stakingCredential()).toPlutusData(), 0, 2, 1, 0);
            case "change-datum" ->
                    field(context, new OutputDatum.OutputDatumInline(PlutusData.integer(0)).toPlutusData(), 0, 2, 1, 2);
            case "fee-theft" -> field(context, Value.lovelace(BigInteger.valueOf(7999999)).toPlutusData(), 0, 2, 1, 1);
            case "unexpected-token" ->
                    field(context, Value.lovelace(BigInteger.valueOf(8000000)).merge(Value.singleton(new PolicyId(new byte[28]), TokenName.EMPTY, BigInteger.ONE)).toPlutusData(), 0, 2, 1, 1);
            case "missing-account-input" -> field(context, new PlutusData.ListData(List.of()), 0, 0);
            case "extra-account-input" ->
                    builder.input(new TxInInfo(AccountFixtures.ref(31), AccountFixtures.output(f.accountAddress, 1))).buildPlutusData();
            case "other-script-input" ->
                    builder.input(new TxInInfo(AccountFixtures.ref(31), AccountFixtures.output(new Address(f.core, Optional.empty()), 1))).buildPlutusData();
    ...
    @ParameterizedTest
    @ValueSource(strings = {"deployment", "account", "version", "state-ref", "protocol", "expired", "wide", "operation", "absent-anchor"})
    void checkpointRejectsSignedForeignOrUnsupportedIntent(String attack) throws Exception {
        var original = f.spend(0);
        var d = original.domain();
        ...
        for (var role : List.of("core", "module")) {
            ...
            context = switch (attack) {
                ...
                case "state-extra-field" -> {
                    var fields = new ArrayList<>(((PlutusData.ConstrData) state).fields());
                    fields.add(PlutusData.integer(0));
                    yield field(context, new PlutusData.ConstrData(0, fields), 0, 1, 0, 1, 2, 0);
                }
                case "state-wrong-tag" ->
                        field(context, new PlutusData.ConstrData(1, ((PlutusData.ConstrData) state).fields()), 0, 1, 0, 1, 2, 0);
                case "module-hash" ->
                        field(context, field(state, PlutusData.bytes(new byte[28]), 5, 0), 0, 1, 0, 1, 2, 0);
                default -> throw new AssertionError(attack);
            };
    ...
    @Test
    void moduleRejectsWrongKeyDuplicateProofsAndWrongScheme() throws Exception {
        var intent = f.spend(0);
        var good = f.authorization(intent);
    ...
    @Test
    void missingOrWrongCheckpointCannotAuthorizeAnAssetInput() throws Exception {
        var intent = f.spend(0);
        var auth = f.authorization(intent);
    ...
    @Test
    void anchorMustActuallyBeLockedByTheAuthenticatedAssetScript() throws Exception {
        var intent = f.spend(0);
    ...
    @ParameterizedTest
    @ValueSource(strings = {"envelope", "domain", "validity", "spend", "recipient", "asset"})
    void malformedSignedRecordsRejectAtTheirOwningImmutableBoundary(String target) throws Exception {
        ...
        for (int index : path)
            selected = selected instanceof PlutusData.ConstrData c ? c.fields().get(index) : ((PlutusData.ListData) selected).items().get(index);
        ...
        var missing = new ArrayList<>(record.fields());
        missing.removeLast();
    ...
    @Test
    void immutableStateCannotBeSpent() throws Exception {
        var intent = f.spend(0);
