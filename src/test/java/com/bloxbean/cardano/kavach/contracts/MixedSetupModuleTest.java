...
class MixedSetupModuleTest {
    static JulcList<Signature> proofs(AccountFixtures f, byte[] digest, int... ids) throws Exception {
        ...
        for (int id : ids)
            values.add(id == 1 ? BrowserModuleTest.proofs(f, 2, digest, id).head() : new Signature(BigInteger.valueOf(id), new byte[0]));
    ...
    void genesis(String attack) throws Exception {
        ...
        if (attack.equals("duplicate"))
            proof = AccountFixtures.list(proof.head(), proof.head(), proof.tail().tail().head());
        if (attack.equals("wrong-method"))
            proof = AccountFixtures.list(new Signature(BigInteger.ZERO, new byte[0]), new Signature(BigInteger.ONE, new byte[0]), new Signature(BigInteger.TWO, new byte[0]));
        var genesis = new GenesisModuleRedeemer(BigInteger.ONE, f.state, proof, JulcList.empty());
        var stateOutput = f.stateInput().resolved();
        if (attack.equals("wrong-custody"))
            stateOutput = new TxOut(f.sink, stateOutput.value(), stateOutput.datum(), Optional.empty());
    ...
    void onlyUnchangedPrecommittedActivation(String attack) throws Exception {
        ...
        if (!attack.equals("missing-witness"))
            ctx.signer(BrowserSignatures.keyHash(AccountFixtures.publicKey(f.keys.get(2))));
        var result = f.evaluate("module", ctx);
