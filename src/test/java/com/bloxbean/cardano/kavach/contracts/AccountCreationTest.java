...
class AccountCreationTest {
     ...
     */
    EvalResult evaluateModule(PlutusData context) {
        return f.evaluate("module", context);
    }
    ...
    @Test
    void creationRequiresCreatorAndAllKeyPossession() throws Exception {
        var genesis = genesis();
    ...
    @Test
    void maximumRegistryAllKeyPossessionFitsCombinedCreationBudget() throws Exception {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"seed", "creator", "quantity", "burn", "extra-mint", "state-address", "state-nft-name", "module-missing", "wrong-genesis-state"})
    void mintRejectsUnboundInitialization(String attack) throws Exception {
        var genesis = genesis();
        var builder = context("nft", genesis);
        var ctx = builder.buildPlutusData();
        ...
        ctx = switch (attack) {
            ...
            case "quantity" ->
                    builder.mint(Value.singleton(new PolicyId(f.state.accountId().policy()), TokenName.EMPTY, BigInteger.TWO)).buildPlutusData();
            case "burn" ->
                    builder.mint(Value.singleton(new PolicyId(f.state.accountId().policy()), TokenName.EMPTY, BigInteger.ONE.negate())).buildPlutusData();
            case "extra-mint" ->
                    builder.mint(Value.singleton(new PolicyId(f.state.accountId().policy()), TokenName.EMPTY, BigInteger.ONE)
            ...
            case "state-nft-name" ->
                    AccountAdversarialTest.field(ctx, PlutusData.bytes(new byte[]{1}), 0, 2, 0, 2, 0, 1, 1);
    ...
    void genesisRejectsInvalidConfigurationDespitePossessionEvidence(String attack) throws Exception {
        ...
        config = switch (attack) {
            case "alias-key" ->
                    AccountAdversarialTest.field(config, PlutusData.bytes(AccountFixtures.publicKey(f.keys.get(0))), 1, 1, 1);
            case "duplicate-id" -> AccountAdversarialTest.field(config, PlutusData.integer(0), 1, 1, 0);
            case "duplicate-member" ->
                    AccountAdversarialTest.field(config, AccountCodec.data(AccountFixtures.policy(1, 1, 1)), 4);
            case "descending-members" ->
                    AccountAdversarialTest.field(config, AccountCodec.data(AccountFixtures.policy(2, 1, 0)), 3);
            case "unknown-member" ->
                    AccountAdversarialTest.field(config, AccountCodec.data(AccountFixtures.policy(1, 15)), 4);
            ...
            case "key-extra" ->
                    AccountAdversarialTest.field(config, extraField(AccountCodec.data(f.config.keys().head())), 1, 0);
            case "policy-extra" ->
                    AccountAdversarialTest.field(config, extraField(AccountCodec.data(f.config.spend())), 2);
            ...
            case "key-tag" ->
                    AccountAdversarialTest.field(config, new PlutusData.ConstrData(1, ((PlutusData.ConstrData) AccountCodec.data(f.config.keys().head())).fields()), 1, 0);
            case "policy-tag" ->
                    AccountAdversarialTest.field(config, new PlutusData.ConstrData(1, ((PlutusData.ConstrData) AccountCodec.data(f.config.spend())).fields()), 2);
            default -> throw new AssertionError(attack);
        };
        var state = new AccountState(BigInteger.ONE, f.state.accountId(), f.deployment, f.state.coreBinding(), BigInteger.ZERO,
    ...
    @Test
    void missingPossessionAndOrdinaryProofCannotInitialize() throws Exception {
        var good = genesis();
        ...
        var wrong = f.authorization(intent).operationProof().get().signatures().head();
