...
class AccountTransferPreparationTest {
    ...
    private Utxo state() throws Exception {
        ...
        result.setInlineDatum(PlutusDataAdapter.toClientLib(AccountCodec.data(f.state)).serializeToHex()); return result;
    }
    private Utxo input() throws Exception { return utxo(f.assetRef, AddressProvider.getEntAddress(f.assetScript, network).toBech32(), List.of(Amount.ada(10))); }
    @Test void authenticatedInputAndRawSpendProofPrepareTheCompleteInvocationChain() throws Exception {
        var intent = f.spend(0); var tx = new Tx();
    ...
    @ParameterizedTest @ValueSource(strings={"nft", "quantity", "datum", "custody", "state-ref", "input-address", "duplicate-input", "wrong-key", "wrong-role", "scheme"})
    void mismatchedResolvedStateInputsAndProofsRejectBeforePreparation(String attack) throws Exception {
        var state = state(); var input = input(); var intent = f.spend(0);
        var proof = f.authorization(intent).operationProof().orElseThrow();
        switch (attack) {
            case "nft" -> state.setAmount(List.of(Amount.ada(10)));
            case "quantity" -> state.setAmount(List.of(Amount.ada(10), Amount.asset(HexUtil.encodeHexString(f.state.accountId().policy()), BigInteger.TWO)));
            ...
            case "wrong-key" -> proof = new Proof(BigInteger.ZERO, AccountFixtures.list(AccountFixtures.sign(0, f.keys.get(1), AccountCodec.intentDigest(intent, null))));
            case "wrong-role" -> proof = new Proof(BigInteger.ZERO, AccountFixtures.list(AccountFixtures.sign(1, f.keys.get(1), AccountCodec.intentDigest(intent, null))));
            case "scheme" -> proof = new Proof(BigInteger.ONE, proof.signatures());
            default -> throw new AssertionError(attack);
        }
        var evidence = proof; var inputs = attack.equals("duplicate-input") ? List.of(input, input) : List.of(input);
    ...
    private JulcList<RewardReceipt> receipts(int moduleIndex) throws Exception {
        var core = new RewardReceipt(f.core, BigInteger.TWO); var module = new RewardReceipt(f.module, BigInteger.valueOf(moduleIndex));
    ...
    @Test void positiveWithdrawalsRequireCompleteDistinctReceipts() throws Exception {
        var intent = f.spend(0); var proof = f.authorization(intent).operationProof().orElseThrow();
    ...
    @Test void wholeInputDigestUsesCanonicalCompleteMapDespiteBackendAmountOrder() throws Exception {
        var input = input(); var quantity = BigInteger.ONE.shiftLeft(64).subtract(BigInteger.ONE);
