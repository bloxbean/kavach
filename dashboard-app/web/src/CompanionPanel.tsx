...
function QR({text}: { text: string }) {
    ...
    useEffect(() => {
        ...
        QRCode.toDataURL(text, {width: 520, margin: 4, errorCorrectionLevel: "L"})
            ...
            .catch(() => {
                if (active) setError("QR is too large. Copy the request and use Import on your phone.");
            });
        return () => {
            active = false;
        };
    ...
    return <>{image &&
        <img className="companion-qr" src={image} alt="Scan with Yano Companion on your iPhone"/>}{error &&
        <p role="alert">{error}</p>}</>;
}

type Exported = { qr: string; expiresAt: number };

export function CompanionPanel({plan, onPlan, disabled, focusedKey}: {
    plan: Plan;
    ...
    useEffect(() => {
        setScanning(false);
    }, [proof?.id, proof?.purpose, disabled]);
    useEffect(() => {
    ...
    async function addApproval(text: string) {
        ...
        const updated = await api<Plan>(`/plans/${plan.id}/companion-proof`, {
    ...
    return <details open={focusedKey ? true : undefined} className="companion-panel" onToggle={e => {
        if (!e.currentTarget.open) setScanning(false);
    }}>
        ...
        <details>
            <summary>Pair a new phone</summary>
            <button className="button" disabled={busy || disabled || scanning}
                    onClick={() => run(async () => setPairing(await api("/companion/pairing")))}>Show dashboard pairing
                QR
            </button>
            {pairing && <section>
                <QR text={pairing.qr}/>
        ...
        {!focusedKey && <label>Phone credential
            <select value={selected} disabled={busy || disabled || scanning} onChange={e => {
                ...
                {proofs.map(p => <option key={`${p.purpose}:${p.id}`} value={`${p.purpose}:${p.id}`}>{p.purpose} ·
                    Key {p.id} · {p.publicKey.slice(0, 12)}…</option>)}
            </select>
        </label>}
        {proof && <>
            {!focusedKey && <code>{proof.publicKey}</code>}
            <button className="button" disabled={busy || disabled || scanning} onClick={() => run(async () => {
                const result = await api<Exported>(`/plans/${plan.id}/companion-request`, {
                    credentialId: proof.id,
                    purpose: proof.purpose
                });
                setRequest(result);
            ...
            {request && <section>
                ...
                <p>Expires {new Date(request.expiresAt).toLocaleTimeString()}. In Companion choose Scan a request. Check
                    the displayed account and policies or payment before approving.</p>
                <div className="workflow-phone-actions">
                    <button className="text-button"
                            onClick={() => run(async () => navigator.clipboard.writeText(request.qr))}>Copy request for
                        phone import
                    </button>
                    {!scanning &&
                        <button type="button" className="button primary" disabled={busy || disabled} onClick={() => {
                            setError("");
                            setScanning(true);
                        }}>Scan phone approval QR</button>}</div>
                {scanning && <ApprovalScanner onClose={() => setScanning(false)} onRead={text => {
                    setScanning(false);
                    void run(() => addApproval(text));
                }}/>}
                {busy && <p role="status">Verifying phone approval…</p>}
                <details>
                    <summary>Paste approval JSON instead</summary>
                    <label>Approval JSON
                        <textarea value={approval} maxLength={8192} onChange={e => setApproval(e.target.value)}
                                  placeholder='Paste the phone’s “Copy approval JSON” here' rows={5}
                                  spellCheck={false}/>
                    </label>
                    <button className="button" disabled={busy || disabled || scanning || !approval.trim()}
                            onClick={() => run(() => addApproval(approval))}>Verify and add pasted approval
                    </button>
                </details>
                <p>This adds a signature to this request. Submit the transaction after all remaining approvals are
                    collected.</p>
            </section>}
        </>}
...
export function CompanionPairing() {
    ...
    return <details className="companion-panel">
        ...
        <p>Open Yano Companion on your phone. Pair this dashboard by scanning its QR and comparing the full fingerprint.
            Then copy the phone’s public key into one of the authority lines below and choose COSE.</p>
        <button type="button" className="button" disabled={busy} onClick={async () => {
            setBusy(true);
            setError("");
            try {
                setPair(await api("/companion/pairing"));
            } catch (e) {
                setError(e instanceof Error ? e.message : String(e));
            } finally {
                setBusy(false);
        ...
        {pair && <><QR text={pair.qr}/><strong>Compare fingerprint on both
            devices</strong><code>{pair.fingerprint}</code></>}
