...
export function approvalPurpose(purpose: string) {
    return ({
        genesis: 'Enroll this key',
        operation: 'Authorize this change',
        candidate: 'Confirm this key in the new policy',
        configuration: 'Confirm this key in the new configuration',
        possession: 'Confirm this key in the new policy'
    } as Record<string, string>)[purpose] || `Approve ${purpose}`;
}
...
export function setupPhase(step: number) {
    return step <= 6 ? 0 : step === 7 ? 1 : 2;
}
