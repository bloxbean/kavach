...
export function ApprovalScanner({onRead, onClose}: { onRead: (text: string) => void; onClose: () => void }) {
                ...
                const scan = () => {
                    ...
                    try {
                        if (source.readyState >= 2 && source.videoWidth && source.videoHeight) {
                            ...
                            if (result?.data) {
                                stop();
                                callbacks.current.onRead(result.data);
                                return;
                            }
                        }
                        timer = setTimeout(scan, 180);
                    } catch {
                        fail("Could not read the camera. Close and try again, or paste the approval JSON.");
                    }
                };
                scan();
            } catch (error) {
    ...
    return <section className="approval-scanner" aria-label="Scan phone approval">
        ...
        <p>Camera frames stay in this browser. Only the decoded approval is sent to your local Kavach backend for
            verification. Scanning adds the approval; it does not submit a transaction.</p>
