...
Local registry regression tests run with `npm test` in `dashboard-app/web` (Node 22.12+).
...
confirmed on-chain accounts. `dashboard-app/backend/data/profiles/` contains public trusted module
...
The backend's ignored `dashboard-app/backend/data/companion-request-key.bin` is a 0600 request
...
The dashboard lives in `dashboard-app/` (formerly `demo/`). Use
`dashboardWebInstall`, `dashboardWebBuild` and
`:dashboard-app:backend:dashboardIntegrationTest` for the current build tasks.
The Java package and protocol signing-domain strings retain their existing names for
compatibility. Historical qualification evidence retains its original paths and commands.

When moving an existing checkout, move the ignored backend data directory too: it contains
